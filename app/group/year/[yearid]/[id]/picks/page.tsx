...
const Page: React.FC<{
  ...
  const pointsScored = useMemo(() => {
    ...
    //get all the choices that correspond to the finished games
  ...
  const modifyChoices = (choice: Choice) => {
    ...
    const existingChoice = choices.find(
      (choice) =>
        choice.bet_id === sampleBets.findIndex((bet) => bet === selectedBet)
    );
...
const GameDisplay: React.FC<GameDisplayProps> = ({ game, bet, choice }) => {
